...
function listDateRange(start, end) {
  const out = [];
  let cursor = start;
  for (let i = 0; i < 4000 && cursor <= end; i += 1) {
    out.push(cursor);
...
function hourFromIsoInZone(isoTimestamp, tz) {
  if (!isString(isoTimestamp) || !ISO_TIMESTAMP_PATTERN.test(isoTimestamp)) {
    return null;
  }
  const date = new Date(isoTimestamp);
  if (Number.isNaN(date.getTime())) return null;
  // Intl.DateTimeFormat with the configured timeZone gives the operator-local
  // hour for an ISO timestamp authored in any zone. We use formatToParts so
  // we can pick the numeric hour without locale-specific separators.
  let formatter;
  try {
...
function aggregate(snapshot, options = {}) {
  ...
  for (const profileId of Object.keys(resultsByProfile)) {
    ...
    for (const dailyKey of Object.keys(entries)) {
      ...
      const hour = hourFromIsoInZone(entry.updatedAt, tz);
      if (hour !== null) {
  ...
  // or before each day in the window.
  ...
  const profileGrowth = dateRange.map((date) => {
    let count = 0;
    for (const created of profileCreations) {
      if (created <= date) count += 1;
      else break;
    }
