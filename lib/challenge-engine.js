...
// Pick the next puzzle the player needs to solve. Returns the puzzle
// or null if all are done.
function findActivePuzzle(session) {
  if (!session || !Array.isArray(session.puzzles)) return null;
  return session.puzzles.find((p) => !p.solved && (p.guesses?.length || 0) < (p.maxGuesses ?? 12)) || null;
}

// Returns the puzzle at the given index, or null if out of range.
// Sessions revel puzzles one at a time: the player only ever sees
...
function projectSessionForPlayer({ session, challenge, now = new Date(), includeAnswersForCompleted = true }) {
  ...
  const puzzles = session.puzzles.map((p) => {
    const out = {
      ...
      guesses: Array.isArray(p.guesses) ? p.guesses.slice() : [],
