...
function normalizeClassRecord(rawClass) {
  ...
  if (memberProfileIds.length > DEFAULT_MAX_MEMBERS_PER_CLASS) {
    memberProfileIds.length = DEFAULT_MAX_MEMBERS_PER_CLASS;
  }
...
function normalizeClassesState(rawState, options = {}) {
  ...
  for (const candidate of rawClasses) {
    const normalized = normalizeClassRecord(candidate);
    if (!normalized) {
      hadInvalidContent = true;
      continue;
    }
...
class ClassesStore {
  ...
  async createClass(name) {
    ...
    return this.#enqueueWrite(async () => {
      ...
      this.state.classes.sort((left, right) => left.createdAt.localeCompare(right.createdAt));
      await this.#persist();
  ...
  async deleteClass(classId) {
    return this.#enqueueWrite(async () => {
      const index = this.state.classes.findIndex((entry) => entry.id === classId);
      if (index === -1) {
  ...
  async deleteClassWithCarveOut(classId) {
    return this.#enqueueWrite(async () => {
      ...
      const carveOutIds = memberIds.filter((id) => !otherActiveMembership.has(id));
      ...
      for (const entry of this.state.classes) {
        ...
        const filtered = entry.memberProfileIds.filter((id) => !carveOutSet.has(id));
  ...
  // Atomically remove any member references that are NOT in the surviving
  // profile-id set, across every class. Used after a leaderboard mutate
  // when the profile-cap normalizer pruned profiles that were previously
