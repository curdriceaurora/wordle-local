...
class ClassesStore {
  ...
  // concurrent reader. Joining the queue means readers see only state
  // that has been durably persisted (or, if the write fails, the
  // restored pre-mutation snapshot from #enqueueWrite's rollback).
  async #awaitPendingWrites() {
    try {
      await this.writeQueue;
    } catch (_err) {
      // The pending write may have rejected; the restore-on-failure path
      // in #enqueueWrite already reverted this.state. Ignore the error so
      // the read can return the (now-consistent) state.
    }
  }
  ...
  // Drop any class member references that aren't in the surviving profile-id
  // set, across every class. Returns one entry per class that was modified
  // with the IDs that were removed. Used to keep class rosters consistent
  // after a leaderboard mutation that pruned profiles (e.g. a non-admin
  // /api/stats/profile call that hit the host cap).
  async reconcileMissingProfiles(survivingProfileIds) {
    return this.#enqueueWrite(async () => {
      const survivingSet =
        survivingProfileIds instanceof Set
          ? survivingProfileIds
          : new Set(Array.isArray(survivingProfileIds) ? survivingProfileIds : []);
      const removedByClass = [];
      const nowIso = this.now().toISOString();
      let touched = 0;
      for (const entry of this.state.classes) {
        const filtered = entry.memberProfileIds.filter((id) => survivingSet.has(id));
        if (filtered.length !== entry.memberProfileIds.length) {
          const dropped = entry.memberProfileIds.filter((id) => !survivingSet.has(id));
          entry.memberProfileIds = filtered;
          entry.updatedAt = nowIso;
          touched += 1;
          removedByClass.push({ classId: entry.id, removedProfileIds: dropped });
        }
      }
      if (touched > 0) {
        await this.#persist();
      }
      return removedByClass;
    });
  }
  ...
  async #persist() {
    ...
    try {
      ...
      if (err && (err.code === "EEXIST" || err.code === "EPERM")) {
        // Move the live file to a backup before retrying so a failed retry
        // doesn't leave the canonical file gone. Restore from backup if the
        // retry rejects; clean up the backup on success.
        ...
        try {
          ...
          if (!backupErr || backupErr.code !== "ENOENT") {
            // If we can't move the live file aside, abandon the retry —
            // leave the original in place rather than risk losing it.
            await fsp.rm(tempPath, { force: true });
            throw backupErr;
          }
          // ENOENT means the live file is already missing; proceed to retry.
        }
        try {
          await fsp.rename(tempPath, this.filePath);
        } catch (retryErr) {
          if (backupCreated) {
            try {
              await fsp.rename(backupPath, this.filePath);
            } catch (restoreErr) {
              // Best effort — the canonical file is now lost. Re-raise the
              // original retry error after surfacing the restore failure.
              this.logger.error(
                `[classes-store] Failed to restore backup after rename retry: ${restoreErr?.message || String(restoreErr)}`
              );
            }
          }
          await fsp.rm(tempPath, { force: true });
