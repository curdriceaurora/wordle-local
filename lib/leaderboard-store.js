...
function normalizeProfile(rawProfile) {
  ...
  if (!name || name.length > 24 || !PROFILE_NAME_PATTERN.test(name)) {
    return null;
  }
...
function normalizeResultEntry(rawEntry, dailyKeyDate) {
  ...
  const maxGuesses = Number(rawEntry.maxGuesses);
  const submissionCount = Number(rawEntry.submissionCount);
...
function normalizeLeaderboardState(rawState, options = {}) {
  const maxProfiles = Number.isInteger(options.maxProfiles)
    ? options.maxProfiles
    : DEFAULT_MAX_PROFILES;
  const maxResultsPerProfile = Number.isInteger(options.maxResultsPerProfile)
  ...
  const version = isPositiveInteger(rawState.version)
    ? Number(rawState.version)
    : DEFAULT_SCHEMA_VERSION;
  if (version !== rawState.version) {
    hadInvalidContent = true;
  }
  ...
  if (profiles.length > maxProfiles) {
    ...
    for (const profile of profiles) {
      if (!keptIds.has(profile.id)) {
        ...
        seenProfileIds.delete(profile.id);
      }
    }
    wasPruned = true;
  }
...
class LeaderboardStore {
  constructor(options = {}) {
    ...
    this.maxProfiles = Number.isInteger(options.maxProfiles)
      ? options.maxProfiles
      : DEFAULT_MAX_PROFILES;
    this.maxResultsPerProfile = Number.isInteger(options.maxResultsPerProfile)
      ? options.maxResultsPerProfile
      : DEFAULT_MAX_RESULTS_PER_PROFILE;
