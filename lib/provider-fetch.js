...
function verifyChecksums(fileRecords, expectedChecksums) {
  if (!expectedChecksums.dic && !expectedChecksums.aff) {
    return;
  }
...
function createManifestPayload(descriptor, retrievedAt, records) {
  return {
    schemaVersion: 1,
    provider: {
...
async function fetchAndPersistProviderSource(options) {
  ...
  const manifestPayload = createManifestPayload(descriptor, retrievedAt, records);
  const manifestPath = path.join(variantRoot, "source-manifest.json");
