...
function parseAcceptLanguage(headerValue) {
  ...
  for (const raw of candidates) {
    ...
    for (let i = 1; i < parts.length; i += 1) {
      // Allow optional sign so headers like `q=-1` are PARSED (and
      // then clamped to 0 by the Math.max below) instead of being
      // silently ignored as malformed — RFC 9110 doesn't define
      // negative q but real-world clients sometimes emit them, and
      // the safer interpretation is "least preferred" rather than
      // "default of 1".
      const m = /^q=(-?[\d.]+)$/i.exec(parts[i].trim());
      if (m) {
        // `q=.` and `q=-` match the regex but parseFloat returns NaN;
        // skip the assignment for non-finite values so malformed
        // q-factors don't poison the sort comparator.
        const parsedQ = Number(m[1]);
...
// Test seam: inject locale messages directly without touching disk.
// Used by server-i18n unit tests to construct fixtures with deliberately
// one-sided keys so the en-fallback path can be exercised. Production
// callers should never use this.
function _setLocaleForTests(locale, messages) {
  if (!cache) cache = Object.create(null);
