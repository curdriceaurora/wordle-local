...
function parseAcceptLanguage(headerValue) {
  ...
  for (const raw of candidates) {
    ...
    for (let i = 1; i < parts.length; i += 1) {
      ...
      if (m) {
        // `q=.` matches the regex but parseFloat returns NaN; clamp
        // to [0,1] only when the parsed value is finite, so malformed
        // q-factors don't make the ranking comparator poison-pill on
        // NaN.
