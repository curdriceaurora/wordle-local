...
function parseBulkNames(input, options = {}) {
  ...
  while (i < maxLength) {
    ...
    // Once a quoted field closes, the only legal next character is a
    // delimiter (comma / CR / LF) or end-of-input. `"Alice"x` is malformed
    // — accepting it as `Alicex` would silently swallow data.
    if (justClosedQuote) {
      if (ch !== "," && ch !== "\r" && ch !== "\n") {
        errors.push({ line, message: "Unexpected character after closing quote." });
        return { names: [], errors };
      }
