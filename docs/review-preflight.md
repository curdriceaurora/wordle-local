...
| 2026-05-11 | #141 | Copilot + investigation | `playwright install --with-deps` unconditionally installed all 3 browser binaries even though `playwright.config.js` skipped webkit (~150MB wasted download per scheduled/dispatched run). | When `DEFAULT_BROWSERS` is scoped, the CI `playwright install` browser list must match. Pin both together; comment on the install step pointing to the tracking issue when the matrix shrinks. |
| 2026-05-11 | #141 | Investigation surfaced via full nightly run | Nightly UI cron had been red for 10+ days because `tests/ui/create-play.spec.js` used a bare `locator('.hint')` that became ambiguous after UI evolution added 3 more `.hint` elements. | Class-wide sweep rule from `CLAUDE.md` already covered this; re-emphasized in PR description. Filed #142 separately for the deeper webkit-browser failures that were also hidden by the nightly-only schedule. |
| *TBD* | *TBD* | *TBD* | *TBD* | *TBD* |
